...
---------------------
* The pair pool contains ALL possible directed pairs on the 9-node graph
  (48 grid + 16 loop + 8 tie = 72 unique pairs).  The original 44 catalog
  pairs from Tables 3-5 are included first; the remaining 28 pairs are
  newly generated by BFS classification of the graph.
* Each trial uses a **unique** pair — no pair is repeated.  If a category
  does not have enough unique pairs to fill its quota, the script aborts
...
* Navigation and crafting each get their own independent 60-trial sequence
  drawn from the same universal pair pool (a pair may appear in both domains
  but never twice within the same domain).
...
class InsufficientPairsError(Exception):
    """Raised when the graph does not have enough unique pairs for a category."""


def _draw_unique_pairs(
    pool: List[PairSpec],
    count: int,
    already_used: set,
    rng: random.Random,
) -> List[PairSpec]:
    """Draw *count* unique pairs from *pool*, excluding *already_used* pair_ids.

    Raises ``InsufficientPairsError`` if not enough unique pairs are available.
    """
    available = [p for p in pool if p.pair_id not in already_used]
    if len(available) < count:
        raise InsufficientPairsError(
            f"Need {count} unique pairs but only {len(available)} remain "
            f"(pool={len(pool)}, already_used={len(already_used)}). "
            f"Category: {pool[0].category if pool else '?'}. "
            f"Total possible on graph: {len(pool)}. "
            f"The 9-node graph does not have enough unique pairs for this "
            f"category to fill the requested trial quota without repeats."
        )
    rng.shuffle(available)
    selected = available[:count]
    for p in selected:
        already_used.add(p.pair_id)
    return selected
...
def _generate_domain_trials(
    ...
    Every trial uses a unique pair — no pair appears twice within a domain.
    Raises ``InsufficientPairsError`` if the graph does not have enough
    unique pairs for any category.
    """
    full_catalog = get_full_pair_catalog()
    rng = random.Random(seed)

    # Track used pair_ids per category within this domain
    used: Dict[Category, set] = {"grid": set(), "loop": set(), "tie": set()}
    ...
    for block_idx, size in enumerate(_SESSION_SIZES):
        ...
        for category in ("grid", "loop", "tie"):
            cat: Category = category  # type: ignore[assignment]
            n = quota[cat]
            pairs = _draw_unique_pairs(
                full_catalog[cat], n, used[cat], rng,
            )
            for pair in pairs:
                global_index += 1
...
def check_feasibility() -> bool:
    """Print a feasibility report and return True if generation is possible."""
    full_catalog = get_full_pair_catalog()

    total_per_cat: Dict[Category, int] = {
        cat: sum(q[cat] for q in [_QUOTA_24, _QUOTA_24, _QUOTA_12])
        for cat in ("grid", "loop", "tie")
    }

    print("=" * 60)
    print("Feasibility check: unique pairs required vs. available")
    print("=" * 60)
    all_ok = True
    for cat in ("grid", "loop", "tie"):
        avail = len(full_catalog[cat])
        need = total_per_cat[cat]
        ok = avail >= need
        status = "OK" if ok else "INSUFFICIENT"
        print(f"  {cat:5s}: need {need:3d} unique, have {avail:3d} on graph -> {status}")
        if not ok:
            all_ok = False
    print()
    if not all_ok:
        print(
            "ERROR: The 9-node graph (3×3 grid + ring) does not have enough\n"
            "unique directed pairs in one or more categories to fill the\n"
            "requested trial quotas (7:7:10 per 24-trial session) without\n"
            "repeating any pair within a single domain.\n"
            "\n"
            "Options to resolve:\n"
            "  1. Adjust the per-session category ratio (reduce tie quota)\n"
            "  2. Allow pair repeats within a domain\n"
            "  3. Use a larger graph with more nodes\n"
        )
    return all_ok
...
def main() -> int:
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only check feasibility without generating",
    )
