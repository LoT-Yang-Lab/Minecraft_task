"""
tail_task_crafting.py — 尾部任务 (Tail Task · Crafting 版) 独立 Flask 应用

与 ``tail_task.py``（navigation 版）结构完全一致：三阶段 / 同一拓扑 / 同一交通命名。
**不同之处**仅在于节点表现：从「彩色几何形状」换成「贴图石块 stone_01..stone_09」，
与 NC_MIX_TASK / crafting 子任务里实际使用的石块贴图一致（资源位于
``Minecraft8.0/shared/assets/stone/stone_0X.png``）。

节点：9 块石头 stone_01 … stone_09，显示为「石块一 … 石块九」 + 对应的 PNG 贴图。
边：与 navigation 版一致的 3 种交通线路（运行时「统一网格」）：
  · 公交 Q (前/向左, 深蓝) + 公交 E (后/向右, 浅蓝)  — 行内一步双向
  · 地铁 A (前/向上, 深绿) + 地铁 D (后/向下, 浅绿)  — 列内一步双向
  · 快速巴士 W（紫） — 仅在 4 角顺时针单向：1 → 3 → 9 → 7 → 1

阶段（与 navigation 完全一致的试次结构）：
  B  — 节点间距离判断（36 试，4 选 1：1 / 2 / 3 / 4 步）+ 5 级置信
         距离按「无向网格图」（公交 + 地铁，不含快速巴士 W）计算，最大 4 步
  D  — 动作序列流畅度（20 试，5 级 Likert）
  E  — 中转节点报告（27 试，9 起点 × 3 终点）

数据保存：data/tail_task_crafting/{subject_id}_{timestamp}.json + .csv

运行：
    python tail_task_crafting.py
...
# ============================================================
# 节点 / 交通线路定义（crafting 节点 + 与 navigation 同名同义的 3 种交通线路）
# ============================================================
# 9 块石头 stone_01..stone_09，按 3×3 行优先排布：
#   stone_01 stone_02 stone_03
#   stone_04 stone_05 stone_06
#   stone_07 stone_08 stone_09
#
# crafting 任务中石头本身无颜色/形状语义，仅以「石块一/二/…/九」标识，
# 但页面会同时贴上对应的 PNG 图（stone_0X.png）作为表现。
_NUM_CN = ("一", "二", "三", "四", "五", "六", "七", "八", "九")

NODES: Dict[int, Dict[str, Any]] = {
    i: {
        "id": i,
        "stone_id": f"stone_{i:02d}",
        "label": f"石块{_NUM_CN[i - 1]}",
        "image": f"stone_{i:02d}.png",
    }
    for i in range(1, 10)
}
...
# 3 种交通线路——与 navigation 版完全一致（同名、同拓扑、同颜色）。
# 每条双向线路实际由 2 个方向键驱动 (Q/E, A/D)，运行时绘成两种深浅色调。
LINES: Dict[str, Dict[str, str]] = {
    "bus": {
        "label": "公交",
        "color": "#3CA0FF",
        "color_q": "#1E5FE6", "color_e": "#6FA8FF",
        "label_q": "公交Q", "label_e": "公交E",
        "key_q": "Q", "key_e": "E",
    },
    "subway": {
        "label": "地铁",
        "color": "#50C878",
        "color_a": "#2E9F4A", "color_d": "#6BD884",
        "label_a": "地铁A", "label_d": "地铁D",
        "key_a": "A", "key_d": "D",
    },
    "rapidbus": {
        "label": "快速巴士",
...
# 别名（向后兼容 —— 旧代码可能引用 P1_EDGES 等）
P1_EDGES = BUS_EDGES
P2_EDGES = SUBWAY_EDGES
P3_NEXT = RAPID_NEXT
...
def _directed_line(a: int, b: int) -> Optional[str]:
    """有向一步关系（类型粒度：返回 'bus' / 'subway' / 'rapidbus' 或 None）。"""
...
# ============================================================
# 距离图：仅公交 + 地铁（无向 3×3 网格），不含快速巴士 W ⇒ 直径 = 4。
...
DATA_DIR = os.path.join(BASE_DIR, "data", "tail_task_crafting")
...
def save_session(subject_id: str, started_at: float,
    ...
    payload = {
        ...
        "ended_at": time.time(),
        "graph_version": "NC_MIX_TASK / crafting (3x3 stones + 4-corner 快速巴士 1-3-9-7-1; keys Q/E/A/D/W)",
...
app.secret_key = "tail_task_crafting_secret_2026"
...
def _node_payload(nid: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "stone_id": n["stone_id"],
        "label": n["label"],
        "image": n["image"],            # 供前端拼 /stone/<image> 贴图
    }
...
# 石块贴图资源目录：Minecraft8.0/shared/assets/stone/stone_0X.png
# 从 tail_task/ 出发上溯 3 层 → NC_MIX_TASK/ → experiments/ → Minecraft8.0/
STONE_DIR = os.path.normpath(os.path.join(
    BASE_DIR, "..", "..", "..", "shared", "assets", "stone"
))


@app.route("/stone/<path:filename>")
def serve_stone(filename: str):
    """向前端提供 shared/assets/stone/stone_0X.png。"""
    return send_from_directory(STONE_DIR, filename)
...
# ============================================================
# 内嵌 HTML（crafting 视觉：石块以中性圆角方块呈现，无颜色/形状语义）
# ============================================================
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* 石块块：PNG 贴图 + 中文标签（节点本身无颜色/形状语义） */
  .stone-tile { display: inline-flex; align-items: center; justify-content: center;
                background: #232732;
                border: 2px solid var(--stone-edge);
                border-radius: 12px;
                box-shadow: 0 2px 6px rgba(0,0,0,0.4);
                overflow: hidden; padding: 4px; }
  .stone-tile img { display: block; width: 100%; height: 100%; object-fit: contain;
                    image-rendering: -webkit-optimize-contrast;
                    image-rendering: pixelated; }
  .stone-with-name { display: inline-flex; flex-direction: column;
                     align-items: center; gap: 6px; }
  .stone-with-name .nm { font-size: 0.78em; color: var(--text-dim); }
...
<div id="legend" class="legend-bar" style="display:none;">
  <span class="legend-item"><span class="legend-line" style="background:#1E5FE6;"></span>Q 键</span>
  <span class="legend-item"><span class="legend-line" style="background:#6FA8FF;"></span>E 键</span>
  <span class="legend-item"><span class="legend-line" style="background:#2E9F4A;"></span>A 键</span>
  <span class="legend-item"><span class="legend-line" style="background:#6BD884;"></span>D 键</span>
  <span class="legend-item"><span class="legend-line" style="background:#C84BD8;"></span>W 键</span>
</div>

<script>
// ────────────────────────────────────────────────
// 石块渲染：从 /stone/<image> 读取 PNG 贴图（素材库 shared/assets/stone/stone_0X.png）
// ────────────────────────────────────────────────
function stoneTile(node, size=56) {
  return `<span class="stone-tile" style="width:${size}px;height:${size}px;">
            <img src="/stone/${node.image}" alt="${node.label}" draggable="false">
          </span>`;
}
...
function render(payload) {
  ...
  if (payload.kind === "trial_B") {
    root.innerHTML = `
      ...
      <div class="node-pair">
        ${stoneBlock(payload.left, 64, false)}
  ...
  if (payload.kind === "trial_E") {
    ...
    root.innerHTML = `
      <div class="progress">${prog}</div>
      <div class="question">现在让你立刻从<b>起点石块</b>走到<b>终点石块</b>。<br>
        你脑子里<b>最先想到要先经过</b>的中间石块是哪块？（只挑 1 个）
      </div>
      <div class="se-display">
        ${stoneBlock(payload.start, 64, false)}
...
if __name__ == "__main__":
    print("=" * 60)
    print("  尾部任务 · Crafting 版 (石块) 启动中...")
    print(f"  数据目录: {DATA_DIR}")
    print(f"  石块贴图: {STONE_DIR}")
    print("  访问:    http://127.0.0.1:5201")
    print("=" * 60)
