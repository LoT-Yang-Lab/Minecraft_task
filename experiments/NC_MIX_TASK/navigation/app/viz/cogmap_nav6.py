...
def _resolve_map_path(filename: str) -> str:
    ...
    if os.path.isabs(filename) and os.path.exists(filename):
        return filename
    from app.paths import maps_dir
...
def compute_nav6_cogmap(
    ...
    -----
    map_path : str 或 None
        地图 JSON 路径（相对 navigation6/maps/ 或绝对路径）。
        与 game 二选一。
...
def render_and_save_cogmap(
    ...
    写入的 SVG 文件绝对路径列表。
    """
    from app.viz.cogmap_plot_nav6 import (
        render_all_plots,
        save_plots_to_dir,
    )
...
if __name__ == "__main__":
    ...
    _nav6_root = _this_file.parents[2]  # app/viz/ -> navigation6
    _nav6_root_str = str(_nav6_root)
    if _nav6_root_str not in sys.path:
        sys.path.insert(0, _nav6_root_str)

    default_output = os.path.join(_nav6_root_str, "outputs", "cogmap")
