...
# ── 确保 Navigation6 项目根在 sys.path（支持直接运行本文件） ──────────
_this_file = Path(__file__).resolve()
_nav6_root = _this_file.parents[2]  # app/experiment/main.py -> navigation6
...
from app.common.station_names import code_to_station_name
from app.common.trajectory_export import export_navigation_session_to_legacy_xlsx

# ── graph9：9 节点图核心逻辑 ─────────────────────────────
from app.experiment.graph9 import (
    NODE_IDS,
    ACTION_NAMES,
    ACTION_KEYS,
    get_next_node,
...
from app.common.transit_action_display import (
...
_VIS_ERROR_FLASH_DURATION = 0.36  # 无效动作时对应线路红色闪烁（秒）
...
class _VisGraphWidget:
    ...
    def __init__(self):
        ...
        self._anim_action = ""
        self._error_flash_action: Optional[str] = None
        self._error_flash_start: float = 0.0

    def set_rect(self, rect: pygame.Rect):
        self.rect = rect

    def start_error_flash(self, action: str) -> None:
        """当前动作不可执行时，让该动作线在示意图上红色闪烁。"""
        if self.is_animating():
            return
        self._error_flash_action = action
    ...
    def start_animation(self, action: str, from_node: int, to_node: int):
        ...
        self._error_flash_action = None
        cx = self.rect.centerx
        cy = self.rect.centery
        self._anim_from_xy = (float(cx), float(cy))
        # 使用动态均匀角度（由上次 draw 计算），回退到旧固定角度
        angle_deg = self._current_angles.get(action, _ACTION_ANGLES.get(action, 0.0))
    ...
    def draw(self, screen: pygame.Surface, font_sm: pygame.font.Font,
        ...
        if self._error_flash_action is not None:
        ...
        err_elapsed = 0.0
        flash_act: Optional[str] = None
        if self._error_flash_action is not None:
            flash_act = self._error_flash_action
        ...
        for act in all_acts:
            ...
            if flash_act is not None and act == flash_act:
                pulse = max(0.0, _math.sin(err_elapsed * _math.pi * 2 * 7.5)) ** 1.4
                color = tuple(
                    int(color[i] * (1.0 - pulse) + _ERROR_RED[i] * pulse)
                    for i in range(3)
                )
            ...
            label_color = _ERROR_RED if flash_act == act else color
            act_surf = font_sm.render(act_label, True, label_color)
    ...
    def _process_action(action: str, source: str = "key"):
        ...
        if phase == PHASE_TRAIN:
            ...
            if dest is not None:
                ...
                vis_widget.start_error_flash(action)
