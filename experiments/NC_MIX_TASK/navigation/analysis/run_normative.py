...
从项目根运行: python -m analysis.run_normative --maps-dir ... --practice-dir ...
"""
import argparse
import json
import os
...
# Navigation6 项目根（analysis/ 的父目录）
_analysis_dir = os.path.dirname(os.path.abspath(__file__))
_nav6_root = os.path.normpath(os.path.join(_analysis_dir, ".."))
if _nav6_root not in sys.path:
    sys.path.insert(0, _nav6_root)


def main():
    parser = argparse.ArgumentParser(description="Navigation6 规范策略：cogmap → QMDP → 行为对比")
    parser.add_argument("--maps-dir", default=None, help="地图目录，默认 navigation6/maps")
    ...
    if args.maps_dir:
        ...
        from app.paths import maps_dir as _maps_dir
        maps_dir = _maps_dir()
    # 默认输出到 navigation6/outputs/analysis/normative，避免混在 analysis 源码目录下
    output_dir = args.output_dir or os.path.join(_nav6_root, "outputs", "analysis", "normative")
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    from app.viz.cogmap_nav6 import compute_nav6_cogmap
    ...
    rl_data_dir = args.rl_data or os.path.join(_nav6_root, "data", "raw", "trajectory")
